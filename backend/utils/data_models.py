...
class ModelConfig(BaseModel):
    hyperparameters: GBRHParams | ElasticNetParams = Field(description="Гиперпараметры модели")
    target: str = Field(description="Целевая переменная")
    model_class: Literal["GradientBoostingRegressor", "ElasticNet"] = Field(description="Класс модели")
...
class ColumnPreprocessing(BaseModel):
    ...
    fillna_policy: Optional[Literal["mean", "mode"]] = Field(description="Политика заполнения пропусков")
    transformations: Optional[Literal["StandardScaler", "MinMaxScaler", "LabelEncoder", "OneHotEncoder"]] = Field(description="Преобразования данных")

class DatasetPreprocessing(BaseModel):
    dataset_preprocessing: list[ColumnPreprocessing] = Field(description="Список преобразований данных")

class RunConfig(BaseModel):
    preprocessing_config: DatasetPreprocessing = Field(description="Конфигурация предобработки данных")
