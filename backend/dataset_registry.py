...
async def get_storage_usage_mb(user_id: str, s3_client_factory: Callable[[], Awaitable[AioBaseClient]]) -> float | None:
    """
    ...
    async with (await s3_client_factory()) as s3:
        response: dict = await s3.list_objects_v2(Bucket=settings.s3_bucket, Prefix=prefix)
...
async def upload_file(user_id: str, file_bytes: bytes, file_name: str, s3_client_factory: Callable[[], Awaitable[AioBaseClient]]) -> str | None:
    """
    ...
    if current_usage + file_size_mb > MAX_STORAGE_MB:
        logger.info(f"Пользователь {user_id} превышает квоту ({round(current_usage, 2)} + {round(file_size_mb, 2)} > {MAX_STORAGE_MB} MB)")
        return None
    ...
    logger.info(f"Пользователь {user_id} загрузил файл {file_name} в хранилище с id {data_id}")
    return data_id


async def delete_file(user_id: str, data_id: str, s3_client_factory: Callable[[], Awaitable[AioBaseClient]]) -> str | None:
    """
    ...
    async with (await s3_client_factory()) as s3:
        ...
        if "Contents" not in response or len(response["Contents"]) == 0:
            logger.info(f"У пользователя {user_id} не найдено файла {data_id}, не удалось удалить")
            return None
...
async def load_dataframe(user_id: str, data_id: str, s3_client_factory: Callable[[], Awaitable[AioBaseClient]]) -> pd.DataFrame | None:
    """
