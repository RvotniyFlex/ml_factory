...
from backend.training import train_regressor_task, save_trained_model
...
async def train_model_endpoint(
    ...
    s3_client_factory: Callable[[], Awaitable[AioBaseClient]] = Depends(get_s3_client_factory),
):
    """
...
@router.post("/predict/{user_id}/{data_id}/{model_name}", status_code=status.HTTP_200_OK)
async def predict_endpoint(
    ...
    try:
        ...
        logger.exception(f"Ошибка при инференсе модели {model_name} пользователя {user_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    
@router.delete("/delete/{user_id}/{data_id}/{model_name}", status_code=status.HTTP_200_OK)
async def delete_model_endpoint(
    ...
    try:
        ...
        return {"user_id": user_id, "data_id": data_id, "model_name": model_name, "status": "deleted"}
    except Exception as e:
        logger.exception(f"Ошибка при удалении модели {model_name} пользователя {user_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))